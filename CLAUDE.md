# Database — Plugin Context for Claude

> Plugin for the **AlfacodeTeam PhpServicePlatform** (Sentinel) kernel.
> Package `alfacode-team/hkm-plugin-database` · namespace `Plugins\Database\` · solves `database.management`
>
...
| `requires` | — |
| `exposes` | `DatabasePort`, `DatabaseConfigurationContract`, `DatabaseConnectionManagerContract` |
| `emits` | — |
...
`config[]` is also what `hkm plugins enable database` seeds into the project's
...
| `DB_HOST` | string | no | — |
| `DB_PORT` | int | no | — |
| `DB_DATABASE` | string | no | — |
| `DB_USERNAME` | string | no | — |
| `DB_PASSWORD` | string | no | — |
| `DB_CHARSET` | string | no | — |
| `DB_SSL_MODE` | string | no | — |
| `DB_SSL_VERIFY` | bool | no | — |
| `DB_SSL_CA` | string | no | — |
| `DB_UNIX_SOCKET` | string | no | — |
| `DB_ENCRYPT` | bool | no | — |
| `DB_TRUST_SERVER_CERT` | bool | no | — |
| `DB_ENABLE_QUERY_LOG` | bool | no | — |
| `DB_POOL_ENABLED` | bool | no | — |
| `DB_POOL_MIN` | int | no | — |
| `DB_POOL_MAX` | int | no | — |
| `DB_POOL_SIZE` | int | no | — |
| `DB_POOL_ACQUIRE_TIMEOUT_MS` | int | no | — |
| `DB_POOL_IDLE_TIMEOUT` | int | no | — |
| `DB_POOL_MAX_LIFETIME` | int | no | — |
| `DB_POOL_VALIDATE` | bool | no | — |
...
The **single concrete implementation** of the kernel `DatabasePort`. The kernel
defines the port; this plugin provides a production adapter over PDO for four
engines:
...
`ConnectionManager` owns named connections. Everything multi-tenant is built on
it: `Plugins\Tenancy` registers a `tenant:<id>` connection per tenant and rebinds
`DatabasePort` for the request, while control-plane plugins pin the **default**
(central) connection so identity I/O is never redirected into a tenant DB.

## PORTABILITY IS THIS PLUGIN'S JOB — AND ITS CONSUMERS' OBLIGATION

The whole point of the port is that a repository written once runs on all four
engines. Two rules carry that:

```
✗ Hand-writing ON DUPLICATE KEY / ON CONFLICT in a repository — call $db->upsert(),
  which compiles the driver-correct statement
✗ CONCAT / SUBSTRING / date functions in repository SQL — compute in PHP and bind,
  or branch explicitly on $db->driver()
```

`upsert($table, $values, $conflictColumns, ?array $updateColumns = null)`:
`null` updates all non-conflict columns, `[]` is insert-if-absent, a subset
updates only those.

`lastInsertId(?string $sequence = null)` — **pass the sequence name on
PostgreSQL**; without it the value is engine-dependent.

## TRANSLATE EVERY \PDOException

No `\PDOException` may escape a repository. Translate it into
`RepositoryException` with `layer:` and `context:` set, `previous: $e`. A raw PDO
exception carries the SQL and often the bound values into the error log and, in
debug, the response.

## CONNECTION POOLING

`DB_POOL_*` governs a pool for long-lived SAPIs (OpenSwoole). Under PHP-FPM the
process dies each request, so pooling buys nothing and `DB_POOL_ENABLED` should
stay off. A pooled connection is **checked out per request and returned** — never
hold one across a coroutine yield you do not control.

```
✗ A static PDO handle or a connection stored on a static property — it leaks
  across requests under OpenSwoole and crosses tenant boundaries
✗ Leaving DB_ENABLE_QUERY_LOG on in production — it retains every statement
```

## MIGRATIONS

Schema is `alfacode-team/let-migrate` only — never Laravel/Doctrine/Symfony
migration classes, never an ORM. Migrations define schema and nothing else: no
routes, no business logic. Every `up()` needs a matching `down()`. Money columns
are `decimal(precision, scale)`, never float.
